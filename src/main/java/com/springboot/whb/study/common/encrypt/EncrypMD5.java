...
public class EncrypMD5 {
    ...
    public static String encrypt(String param) {
        try {
            ...
            byte[] algorithmArr = param.getBytes();
            //获得MD5摘要算法的MessageDigest对象
            MessageDigest algorithm = MessageDigest.getInstance("MD5");
            algorithm.reset();
            //MessageDigest对象通过使用update方法处理数据，使用指定的byte数组更新摘要
            algorithm.update(algorithmArr);
            //摘要更新后，通过digest()执行哈希计算，获得密文
    ...
    public static void main(String[] args) {
        EncrypMD5 md5Tool = new EncrypMD5();
        System.out.println("EncrypMD5.getMD5ofStr方法执行的结果：" + md5Tool.getMD5ofStr("01722443022419790608551920180627143035780SA$DFJLK%@1AS221%"));
        System.err.println("加密后：" + md5Tool.getMD5ofStr("01722443022419790608551920180627143035780SA$DFJLK%@1AS221%"));
        Date nowTime = new Date();
        String name = "whb123";
        String result = encrypt("01722443022419790608551920180626100936635SA$DFJLK%@1AS221%");
        System.out.println("加密后：" + result);
        System.out.println("当前时间戳一：" + System.currentTimeMillis());
        System.out.println("当前时间戳二：" + Calendar.getInstance().getTimeInMillis());
        System.out.println("当前时间戳三：" + nowTime.getTime());
        System.out.println("当前时间戳四：" + new SimpleDateFormat("yyyyMMddHHmmssSSS").format(nowTime));
        StringBuffer tmp = new StringBuffer();
        tmp.append("017224").append("430224197906085519").append(new SimpleDateFormat("yyyyMMddHHmmssSSS").format(new Date())).append("SA$DFJLK%@1AS221%");
        result = encrypt(tmp.toString());
        System.out.println("单点登录的签名：" + result);
    }
}
